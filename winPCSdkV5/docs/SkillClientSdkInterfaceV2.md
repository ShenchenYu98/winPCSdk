...
3. SDK 调用 Skill 服务端"发送到 IM"接口，传入 `welinkSessionId` 和从缓存中获取的 `content`
...
|---------|---------|
| `text.delta` | 追加临时缓存 |
| `text.done` | 落定最终内容，更新缓存 |
| `thinking.done` | 可选：是否计入最终文本由上层决定 |
| `step.done` | 标记当前步骤完成，缓存已就绪 |

### 注意事项

- SDK 不维护选区本身，只接收或缓存最终文本
- 上层可在用户确认前编辑缓存中的文本，再调用 `sendMessageToIM` 发送
...
try {
  ...
  console.error("发送到 IM 失败:", error.message);
