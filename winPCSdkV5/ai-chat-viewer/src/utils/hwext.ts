...
function normalizeSessionMessageContent(
  ...
  return {
    ...
    parts: (message.parts ?? []).map((part) => ({
      ...
      toolStatus: 'toolStatus' in part ? normalizeToolStatus(part.toolStatus) : undefined,
...
function normalizeStreamMessage(message: StreamMessage | PcStreamMessage): StreamMessage {
  return {
    ...
    messages: message.messages?.map((item) => ({
      id: String(item.id),
      seq: 'messageSeq' in item ? item.messageSeq : item.seq,
      role: item.role,
      ...
      parts: (item.parts ?? []).map((part) => ({
        ...
        toolName: part.toolName,
        toolCallId: part.toolCallId,
        status: 'toolStatus' in part
          ? normalizeToolStatus(part.toolStatus)
          : ('status' in part ? normalizeToolStatus(part.status) : undefined),
        header: part.header,
        question: part.question,
        options: part.options,
        fileName: part.fileName,
